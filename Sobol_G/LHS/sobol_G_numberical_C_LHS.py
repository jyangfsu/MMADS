...
This scripts generates the results of the two sensitivity measures (mean and 
    ...
    The results is 21.32 and 4.91 used in Table 1.
...
N = 10000
mean_C = np.zeros(len(range(10, N, 10)))
var_C = np.zeros(len(range(10, N, 10)))
for idn, n in enumerate(range(10, N, 10)):\
    Y = cmpt_Y_C(n)
    dC, dC2 = diff_C(Y)
    mean_C[idn] = np.mean(dC)
    var_C[idn] = np.var(dC)
    if n % 100 ==0:
        print('    n = %d. mean_C = %.4f. var_C = %.4f' %(n, mean_C[idn], var_C[idn]))
             
# Save mean and variacne  
np.save('mean_C_LHS_' + str(N) + '.npy', mean_C)
np.save('var_C_LHS_' + str(N) + '.npy', var_C)

# Plot the convergence
plt.figure(figsize=(8, 3))
plt.plot(range(10, N, 10), mean_C, label='Direct Monte Carlo')
...
plt.savefig('convergence_C_LHS.png', dpi=300)
plt.show()
'''
