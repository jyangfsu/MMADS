...
binnumbers = np.zeros((Ma, Mb, N))
for i in range(Ma):
    for k in range(Mb):
        print('i = %d, k=%d.' %(i, k))
        if (i==0 and k==0):
            statistic, _, _, binnumbers[i, k, :] = stats.binned_statistic_2d(param_values[:, 0], param_values[:, 2], 
                                                                             np.zeros(N), 
                                                                             'count', 
                                                                             bins=[bin_a, bin_hk],
                                                                             expand_binnumbers=False) 
            
            if len(np.unique(binnumbers[i, k, :])) != nbins_per_dim**2:
                print('    Warnning: Number of empty bins = %d' \
                      %(nbins_per_dim**2 - len(np.unique(binnumbers[i, k, :]))))
            else:
                print('    No empty bins!')
        ...
        if (i==0 and k==1):
           statistic, _, binnumbers[i, k, :] = stats.binned_statistic_dd(param_values[:, [0, 3, 4]], 
                                                                            np.zeros(100),
                                                                            'count', 
                                                                            bins=[bin_a, bin_hk1, bin_hk2],
                                                                            expand_binnumbers=False)
                
           if len(np.unique(binnumbers[i, k, :])) != nbins_per_dim**3:
               print('    Warnning: Number of empty bins = %d' \
                     %(nbins_per_dim**3 - len(np.unique(binnumbers[i, k, :]))))
           else:
               print('    No empty bins!')
        ...
        if (i==1 and k==0):
           statistic, _, _, binnumbers[i, k, :] = stats.binned_statistic_2d(param_values[:, 1], param_values[:, 2], 
                                                                             np.zeros(N), 
                                                                             'count', 
                                                                             bins=[bin_b, bin_hk],
                                                                             expand_binnumbers=False) 
        ...
        if (i==1 and k==1):
           statistic, _, binnumbers[i, k, :] = stats.binned_statistic_dd(param_values[:, [1, 3, 4]], 
                                                                            np.zeros(100),
                                                                            'count', 
                                                                            bins=[bin_b, bin_hk1, bin_hk2],
                                                                            expand_binnumbers=False)
                
           if len(np.unique(binnumbers[i, k, :])) != nbins_per_dim**4:
               print('    Warnning: Number of empty bins = %d' \
                     %(nbins_per_dim**3 - len(np.unique(binnumbers[i, k, :]))))
           else:
               print('    No empty bins!')
...
@nb.njit
def Bin_Process_C(Y):
    bin_Y = np.full((Ma, Mb, max_bins_total, Mc, max_points_per_bin), np.nan)
    for i in range(Ma):
        for k in range(Mb):
            for m in range(Mc):
                binnumber = binnumbers[i, k, :]
                for inumber, number in enumerate(np.unique(binnumber)):
                        bin_Y[i, k, inumber, m, :binnumber[binnumber==number].shape[0]] = Y[i, k, m, :][binnumber==number]
                    
    return bin_Y

@nb.njit
def MMDS_mean_var_C(bin_Y):
    D = np.full((Ma, Mb, max_bins_total, Mc, max_points_per_bin, Mc, max_points_per_bin), np.nan)
    for i in range(Ma):
        for k in range(Mb):
            for inumber in range(max_bins_total):
                for m1 in range(Ma):
                    for n1 in range(max_points_per_bin):
                        for m2 in range(Ma):
                            for n2 in range(max_points_per_bin):
                                D[i, k, inumber, m1, n1, m2, n2] = abs(bin_Y[i, k, inumber, m1, n1] - bin_Y[i, k, inumber, m2, n2])
...
bin_Y = Bin_Process_C(Y)
E_C, V_C = MMDS_mean_var_C(bin_Y)

print('Mean C =', E_C, 'Var C =', V_C)
