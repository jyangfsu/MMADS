...
def cmpt_Y_A(N):
    ...
    # Sampling from the stratum using LHS
    problem = {'nvars': 3,
               'names': ['x1', 'y1', 'z1'],
               'bounds': [[0, 1], [0, 1], [0, 1]],
               'dists' :['UNIFORM', 'UNIFORM', 'UNIFORM']
               }
    param_values = mc(problem, N, seed=933090934)
...
np.save('dA_MC_' + str(N) + '.npy', dA)
np.save('dA2_MC_' + str(N) + '.npy', dA2)
...
np.save('mean_A_MC_' + str(N) + '.npy', mean_A)
np.save('var_A_MC_' + str(N) + '.npy', var_A)
...
plt.savefig('convergence_A_MC.png', dpi=300)
plt.show()
'''
